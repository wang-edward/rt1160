import os
from SCons.Script import Environment, Default, Alias

BUILD_DIR = 'build'
os.makedirs(BUILD_DIR, exist_ok=True)

gcc = '/opt/homebrew/bin/arm-none-eabi-gcc-8.3.1'

env = Environment(
    ENV       = os.environ,
    CC        = gcc,
    AS        = gcc,
    AR        = 'arm-none-eabi-ar',
    OBJCOPY   = 'arm-none-eabi-objcopy',
    CFLAGS    = [
        '-mcpu=cortex-m7','-mfloat-abi=hard','-mfpu=fpv5-d16',
        '-DCPU_MIMXRT1166DVM6A_cm7',
        '-O0','-g3','-ffunction-sections','-fdata-sections',
    ],
    LINKFLAGS = [
        '-T','linker/evk1160.ld',
        '-nostartfiles',
        '-nodefaultlibs',
        '-Wl,--gc-sections',
        '-Wl,-e,Reset_Handler',
        '-g'
    ],
    CPPPATH = [
        'sdk/CMSIS/Core/Include',
        'sdk/devices/MIMXRT1166',
        'sdk/devices/MIMXRT1166/periph',
        'sdk/devices/MIMXRT1166/CMSIS/Include',
        'sdk/devices/MIMXRT1166/gcc',
        'sdk/drivers',
    ],
)

# List your source files
srcs = [
    'src/startup_MIMXRT1166_cm7.S',
    'src/main.c',
    'sdk/drivers/fsl_clock.c',
    'sdk/drivers/fsl_pmu.c',
    'sdk/drivers/fsl_anatop_ai.c',
    'sdk/drivers/fsl_gpio.c',
    'sdk/devices/MIMXRT1166/system_MIMXRT1166_cm7.c',
]

# Compile each source into build/<basename>.o
objs = []
for src in srcs:
    base = os.path.splitext(os.path.basename(src))[0]
    tgt = os.path.join(BUILD_DIR, base + '.o')
    objs.append(env.Object(target=tgt, source=src))

# Link all objects into the final ELF
elf = env.Program(target=os.path.join(BUILD_DIR, 'rt1160_app.elf'),
                  source=objs)

# Also produce a .bin
env.Command(os.path.join(BUILD_DIR, 'rt1160_app.bin'),
            elf,
            '$OBJCOPY -O binary $SOURCE $TARGET')

Default(elf)

# Flash & debug aliases
Alias('flash', None,
      f'pyocd flash -t mimxrt1160 {BUILD_DIR}/rt1160_app.elf')
Alias('gdb', None,
      f'arm-none-eabi-gdb {BUILD_DIR}/rt1160_app.elf -ex "target remote :3333"')
